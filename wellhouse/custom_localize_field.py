...
# class LocalizedInlineSelectPanel(InlinePanel):
#     def __init__(self,relation_name, max_num=None, min_num=None, label="", heading="", panels=None, widget_class=None, typed_choice_field=False, *args, **kwargs):
#         if not widget_class in [None, CheckboxSelectMultiple, RadioSelect, Select, SelectMultiple]:
#             raise ImproperlyConfigured(_(
#                 "widget_class should be a Django form widget class of type "
#                 "CheckboxSelectMultiple, RadioSelect, Select or SelectMultiple"
...
#             super().__init__(**kwargs)
#             if not self.panel.widget_class:
#                 self.form.fields[self.relation_name].widget.choices=self.choice_list
#             else:
#                 self.form.fields[self.relation_name].widget = self.panel.widget_class(choices=self.choice_list)
#             if self.panel.typed_choice_field:
#                 self.form.fields[self.relation_name].__class__.__name__ = 'typed_choice_field'
#             pass
#
#         # @property
#         # def choice_list(self):
#         #     self.form.fields[self.relation_name].queryset = self.form.fields[self.relation_name].queryset.filter(locale_id=self.instance.locale_id)
#         #     choices = ModelChoiceIterator(self.form.fields[self.relation_name])
#         #     return choices
